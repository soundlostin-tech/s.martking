...
export function BottomNav() {
  const pathname = usePathname();
    ...
    return (
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-t border-shadow-grey-100 pb-safe shadow-[0_-5px_20px_-10px_rgba(30,30,36,0.05)]">
              ...
              >
                ...
                >
                  <div className={`relative z-10 p-1.5 rounded-2xl transition-all duration-300 ${
                    isActive ? "bg-secondary/10 text-secondary" : "text-shadow-grey-400"
                  }`}>
                    <Icon size={20} strokeWidth={isActive ? 2.5 : 2} />
                  </div>
  
                  <span className={`text-[9px] font-bold uppercase tracking-wider transition-all duration-300 ${
                    isActive ? "text-secondary scale-110" : "text-shadow-grey-300"
                  }`}>
                    {item.label}
                  </span>
