...
export function AdminNav() {
  const pathname = usePathname();
    ...
    return (
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-2xl bg-background/80 backdrop-blur-2xl border border-dark-slate-grey/5 shadow-[0_20px_50px_rgba(36,62,54,0.1)] rounded-[2.5rem] px-4 overflow-hidden">
                ...
                <div className="relative flex flex-col items-center gap-1.5 transition-all duration-300 py-3">
                    <motion.div
                      ...
                      className={`relative z-10 p-2.5 rounded-2xl transition-colors ${
                        isActive ? "text-accent-foreground bg-accent" : "text-dark-slate-grey/30 hover:text-dark-slate-grey/60"
                      }`}
                    >
                    ...
                    <span className={`text-[9px] font-bold uppercase tracking-[0.2em] transition-colors ${
                      isActive ? "text-accent" : "text-dark-slate-grey/20"
                    }`}>
                      {item.label}
                    </span>
                </div>
