      ...
      <div className="min-h-screen bg-background text-foreground">
        <main className="pb-32 relative z-10">
                    ...
                    <div className="w-full h-full rounded-[21px] bg-background p-0.5">
                      <div className="w-full h-full rounded-[19px] bg-seashell-200 flex items-center justify-center overflow-hidden">
                        {user.user_metadata?.avatar_url ? (
                          ...
                          <span className="text-xl font-heading text-evergreen-500">{user.email?.[0].toUpperCase()}</span>
                        )}
                      </div>
                    </div>
                    {!stories.some(s => s.user_id === user.id) && (
                        ...
                        <div className="w-full h-full rounded-[21px] bg-background p-0.5">
                          <div className="w-full h-full rounded-[19px] bg-seashell-100 flex items-center justify-center overflow-hidden">
                            {profile.avatar_url ? (
                              ...
                              <span className="text-xl font-heading text-evergreen-500">{profile.full_name?.[0].toUpperCase()}</span>
                            )}
                          </div>
                        </div>
                    </motion.div>
                    <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${
                      hasStory ? 'text-sea-green-500' : 'text-evergreen-400'
                    }`}>
          ...
          <section className="px-6 mb-10">
            <div className="relative h-56 rounded-[40px] overflow-hidden shadow-xl shadow-evergreen-900/10 group cursor-pointer active:scale-[0.98] transition-all duration-500 border border-evergreen-100 premium-gradient">
              {/* Ambient FX */}
              <div className="absolute inset-0 overflow-hidden opacity-40">
                <motion.div
                  ...
                  className="absolute -top-20 -left-20 w-64 h-64 bg-malachite-400/20 blur-[80px] rounded-full"
                />
              </div>
              ...
              <div className="relative h-full p-10 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-[10px] font-bold text-malachite-300 uppercase tracking-[0.4em]">ELITE DEPLOYMENT ACTIVE</span>
                </div>
                <h2 className="text-4xl font-heading text-white leading-[1.1] tracking-tight">Win ₹1,00,000<br/><span className="text-malachite-400">Mega Pool</span></h2>
                <p className="text-[11px] text-seashell-200 font-bold uppercase tracking-[0.2em] mt-4 flex items-center gap-2">
                  <Users size={14} className="text-malachite-400" /> 2,450 WARRIORS REGISTERED
                </p>
            ...
            {[
              ...
              >
                <div className="w-10 h-10 rounded-2xl bg-malachite-500/10 flex items-center justify-center mb-4">
                  <stat.icon size={20} className="text-malachite-500" />
                </div>
                <p className="text-[9px] font-bold text-evergreen-400 uppercase tracking-widest mb-1">{stat.label}</p>
          ...
          <section className="px-8 pb-6 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-[10px] font-bold text-evergreen-400 uppercase tracking-[0.3em]">BATTLE SIGNALS DETECTED</p>
            </div>
            <Link href="/matches" className="p-3 bg-card rounded-2xl text-evergreen-500 hover:text-malachite-600 shadow-sm transition-all border border-border">
              <LayoutGrid size={20} />
            </Link>
          </section>
          ...
          <section className="px-6 space-y-5">
            {featuredMatches.length > 0 ? featuredMatches.map((match, i) => (
              ...
              >
                <div className="relative w-24 h-24 rounded-3xl bg-seashell-100 overflow-hidden flex-shrink-0 shadow-inner border border-border">
                   {match.status === 'live' ? (
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-12 h-12 rounded-full bg-malachite-500/10 flex items-center justify-center animate-pulse">
                        <Play size={24} className="text-malachite-500 translate-x-0.5" fill="currentColor" />
                      </div>
                    </div>
                  ) : (
                    <div className="absolute inset-0 flex items-center justify-center opacity-10">
                      <Swords size={32} className="text-evergreen-500" />
                    </div>
                  )}
                  <div className="absolute bottom-2 left-2 right-2 px-2 py-1 rounded-xl bg-white/80 backdrop-blur-md border border-border flex items-center justify-center gap-1.5">
                    <div className={`w-1 h-1 rounded-full ${match.status === 'live' ? 'bg-malachite-500 animate-pulse' : 'bg-evergreen-400'}`} />
                    <span className="text-[8px] font-bold uppercase tracking-tighter text-evergreen-500">{match.status}</span>
                  </div>
                </div>
                
                <div className="flex-1 min-w-0 space-y-2">
                  <p className="text-[10px] font-bold text-evergreen-400 uppercase tracking-[0.2em]">{match.mode} • {match.map || 'Bermuda'}</p>
                  <h3 className="text-xl font-heading text-foreground truncate group-hover:text-sea-green-600 transition-colors">{match.title}</h3>
                  
                  <div className="flex items-center gap-5 pt-1">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-lg bg-malachite-500/10 flex items-center justify-center">
                        <Trophy size={12} className="text-malachite-500" />
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-lg bg-seashell-200 flex items-center justify-center">
                        <Users size={12} className="text-evergreen-500" />
                      </div>
                      <span className="text-[11px] font-bold text-evergreen-500">{match.live_stats?.players_alive || 48} / {match.tournament?.slots || 100}</span>
                    </div>
                  </div>
                </div>
  
                <div className="flex flex-col items-end justify-center gap-1 pl-4 border-l border-border">
                  <span className="text-[9px] font-bold text-evergreen-400 uppercase tracking-[0.3em]">ENTRY</span>
                  <span className="text-2xl font-heading text-sea-green-600">₹{match.tournament?.entry_fee}</span>
                </div>
              </motion.div>
            )) : (
              <div className="py-20 text-center flex flex-col items-center gap-4 bg-seashell-100 rounded-[40px] border border-dashed border-border shadow-sm">
                <Signal size={48} strokeWidth={1} className="text-evergreen-200" />
                <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-evergreen-400 italic">Scanning for Battle Signals...</p>
              </div>
            )}
          </section>
