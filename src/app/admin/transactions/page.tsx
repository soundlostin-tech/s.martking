  ...
  return (
    <main className="min-h-screen pb-32 bg-evergreen-950 bg-[radial-gradient(circle_at_50%_0%,_#0D2818_0%,_#0a2013_100%)]">
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between px-2">
            ...
            <Button 
              variant="ghost" 
              size="sm" 
              className="text-[10px] font-bold text-white/40 hover:text-malachite-500 uppercase tracking-[0.2em]"
                ...
                {filteredTransactions.length > 0 ? (
                  filteredTransactions.map((tx, idx) => (
                    <motion.div 
                      ...
                      className="bg-white/5 backdrop-blur-xl rounded-[2.5rem] p-6 flex flex-col md:flex-row justify-between md:items-center border border-white/10 hover:border-malachite-500/30 hover:bg-white/[0.08] transition-all duration-500 group cursor-pointer"
                    >
                      <div className="flex items-center gap-6">
                        <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center transition-transform duration-500 group-hover:scale-110 bg-white/5 text-white/20 group-hover:text-malachite-500`}>
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-10 mt-6 md:mt-0">
                        <div className="text-right">
                          <p className={`text-2xl font-heading ${['deposit', 'prize'].includes(tx.type) ? "text-malachite-500" : "text-white/40"}`}>
                        ...
                        <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-white/20 group-hover:bg-malachite-500 group-hover:text-white transition-all">
      ...
      <Sheet open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <SheetContent className="bg-evergreen-950 border-white/10 w-full sm:max-w-xl p-0 overflow-y-auto no-scrollbar text-white">
          {selectedTx && (
            <div className="flex flex-col h-full">
              <div className="p-10 bg-evergreen-800 border-b border-white/10 relative overflow-hidden">
                ...
                <div className="absolute top-[-20%] right-[-10%] w-[60%] h-[60%] bg-malachite-500/20 blur-[100px] rounded-full" />
              </div>
              ...
              {selectedTx.type === 'withdrawal' && selectedTx.status === 'pending' && (
                <div className="p-10 bg-evergreen-800 border-t border-white/10 shadow-2xl">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Button 
                      disabled={processingId !== null}
                      className="h-16 rounded-[2rem] bg-malachite-500 hover:bg-malachite-500/80 text-white font-bold uppercase tracking-[0.2em] text-[10px] gap-3 shadow-2xl shadow-malachite-500/20 border-none"
