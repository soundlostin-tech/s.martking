    ...
    <main className="min-h-screen pb-32 bg-background text-white">
      ...
      <div className="px-6 pt-24 relative z-10 space-y-10 max-w-6xl mx-auto">
        ...
        <div className="space-y-1">
          <h4 className="text-[10px] font-bold text-malachite-500 uppercase tracking-[0.4em]">Financial Command</h4>
          ...
          {[
            ...
            >
              <div className="flex justify-between items-start relative z-10">
                <div className="space-y-1">
                  <p className={`text-[10px] uppercase font-bold tracking-[0.2em] ${stat.primary ? "text-malachite-400" : "text-evergreen-500"}`}>{stat.label}</p>
                  <h3 className="text-3xl font-heading text-white">₹{stat.value.toLocaleString()}</h3>
                </div>
                <div className={`p-4 rounded-2xl ${stat.primary ? "bg-malachite-500 text-black shadow-lg shadow-malachite-500/20" : "bg-evergreen-950 text-malachite-400"} group-hover:scale-110 transition-transform duration-500 border border-white/5`}>
                  <stat.icon size={24} />
                </div>
              </div>
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between px-2">
            <div className="space-y-1">
              ...
              <p className="text-[10px] font-bold text-evergreen-500 uppercase tracking-[0.2em]">{filteredTransactions.length} ENTRIES DETECTED</p>
            </div>
            <Button 
              variant="ghost" 
              size="sm" 
              className="text-[10px] font-bold text-evergreen-500 hover:text-malachite-500 uppercase tracking-[0.2em]"
                ...
                {filteredTransactions.length > 0 ? (
                  filteredTransactions.map((tx, idx) => (
                    <motion.div 
                      ...
                      className="bg-evergreen-900/40 rounded-[2.5rem] p-6 flex flex-col md:flex-row justify-between md:items-center border border-evergreen-800 hover:border-malachite-500/30 hover:bg-evergreen-900/60 transition-all duration-500 group cursor-pointer shadow-md"
                    >
                      <div className="flex items-center gap-6">
                        <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center transition-transform duration-500 group-hover:scale-110 bg-evergreen-950 text-evergreen-600 group-hover:text-malachite-500 shadow-inner border border-white/5`}>
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-10 mt-6 md:mt-0">
                        <div className="text-right">
                          <p className={`text-2xl font-heading ${['deposit', 'prize'].includes(tx.type) ? "text-malachite-400" : "text-evergreen-500"}`}>
                            {['deposit', 'prize'].includes(tx.type) ? "+" : "-"}₹{Number(tx.amount).toLocaleString()}
                          </p>
                          <p className="text-[10px] text-evergreen-600 font-bold uppercase tracking-widest">ID: {tx.id.slice(0, 8)}</p>
                        </div>
                        <div className="w-12 h-12 rounded-2xl bg-evergreen-950 flex items-center justify-center text-evergreen-600 group-hover:bg-malachite-500 group-hover:text-black transition-all border border-white/5 shadow-inner">
                  ...
                  <div className="flex flex-col items-center justify-center py-32 bg-evergreen-900/40 rounded-[3rem] border border-dashed border-evergreen-800 text-center shadow-sm">
                    <AlertCircle size={64} strokeWidth={1} className="text-evergreen-800 mb-6" />
                    <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-evergreen-600">No matching signals found</p>
                    <Button variant="link" onClick={() => { setSearch(""); setTypeFilter("all"); setStatusFilter("all"); }} className="text-malachite-500 font-bold mt-4 text-[10px] tracking-widest uppercase">
                      CLEAR ALL FILTERS
                    </Button>
                  </div>
                )}
          ...
          {selectedTx && (
            <div className="flex flex-col h-full">
              <div className="p-10 bg-evergreen-900 border-b border-evergreen-800 relative overflow-hidden">
                <SheetHeader className="relative z-10">
                  ...
                  <div className="space-y-4">
                    <p className="text-evergreen-500 text-[10px] font-bold uppercase tracking-[0.3em]">AUDIT RECORD</p>
                    <SheetTitle className="text-white text-4xl font-heading leading-tight">Transaction Detail</SheetTitle>
                    <p className="text-evergreen-600 text-[10px] font-bold uppercase tracking-[0.2em] font-mono">#{selectedTx.id}</p>
                  </div>
                </SheetHeader>
              ...
              <div className="p-10 space-y-12 flex-1">
                {/* Financial Overview */}
                <div className="bg-evergreen-900/40 p-10 rounded-[2.5rem] border border-evergreen-800 flex flex-col items-center text-center gap-4 shadow-2xl">
                  <p className="text-[10px] text-evergreen-500 uppercase font-bold tracking-[0.3em]">TRANSACTION VALUE</p>
                ...
                <div className="space-y-10">
                  <div>
                    <h4 className="text-[10px] text-evergreen-500 uppercase font-bold tracking-[0.3em] ml-2 mb-4">DOSSIER DETAILS</h4>
                      ...
                      {[
                        ...
                        <div key={i} className="flex items-center justify-between p-6 hover:bg-white/[0.02] transition-colors">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-xl bg-evergreen-950 flex items-center justify-center text-evergreen-600 border border-white/5 shadow-inner">
                              <item.icon size={18} />
                            </div>
                  ...
                  {selectedTx.metadata && (
                    <div>
                      <h4 className="text-[10px] text-evergreen-500 uppercase font-bold tracking-[0.3em] ml-2 mb-4">TECHNICAL LOG</h4>
                      <div className="bg-black/40 rounded-[2.5rem] p-8 text-evergreen-600 font-mono text-[10px] leading-loose break-all border border-evergreen-800">
                ...
                <div className="p-10 bg-evergreen-900 border-t border-evergreen-800 shadow-2xl">
                  <div className="grid grid-cols-2 gap-4">
                    <Button 
                      ...
                      className="h-16 rounded-[2rem] border-evergreen-800 bg-evergreen-950 text-white font-bold uppercase tracking-[0.2em] text-[10px] gap-3 hover:bg-red-950 hover:text-red-400"
