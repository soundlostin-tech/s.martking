...
export default function AdminTransactions() {
  ...
  const getTypeBadge = (type: string) => {
    ...
    switch (type) {
      case 'deposit': return <Badge className={`${baseClass} bg-light-coral/20 text-light-coral`}>DEPOSIT</Badge>;
      case 'withdrawal': return <Badge className={`${baseClass} bg-white/10 text-white/60`}>WITHDRAWAL</Badge>;
      case 'prize': return <Badge className={`${baseClass} bg-light-coral text-white`}>PRIZE</Badge>;
  ...
  const getStatusBadge = (status: string) => {
    const baseClass = "text-[9px] font-bold px-3 py-1 rounded-full border-none shadow-sm";
    switch (status) {
      case 'completed': return <Badge className={`${baseClass} bg-light-coral text-white`}>COMPLETED</Badge>;
    ...
    <main className="min-h-screen pb-32 bg-yale-blue bg-[radial-gradient(circle_at_50%_0%,_#355070_0%,_#304966_100%)]">
      ...
      <div className="px-6 pt-24 relative z-10 space-y-10 max-w-6xl mx-auto">
        ...
        <div className="space-y-1">
          <h4 className="text-[10px] font-bold text-light-coral uppercase tracking-[0.4em]">Financial Command</h4>
        ...
        <div className="space-y-6">
          <div className="flex items-end justify-between px-2">
            ...
            <Button 
              variant="ghost" 
              size="sm" 
              className="text-[10px] font-bold text-white/40 hover:text-light-coral uppercase tracking-[0.2em]"
                ...
                {filteredTransactions.length > 0 ? (
                  filteredTransactions.map((tx, idx) => (
                    <motion.div 
                      ...
                      className="bg-white/5 backdrop-blur-xl rounded-[2.5rem] p-6 flex flex-col md:flex-row justify-between md:items-center border border-white/10 hover:border-light-coral/30 hover:bg-white/[0.08] transition-all duration-500 group cursor-pointer"
                    >
                      <div className="flex items-center gap-6">
                        <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center transition-transform duration-500 group-hover:scale-110 bg-white/5 text-white/20 group-hover:text-light-coral`}>
                      ...
                      <div className="flex items-center justify-between md:justify-end gap-10 mt-6 md:mt-0">
                        <div className="text-right">
                          <p className={`text-2xl font-heading ${['deposit', 'prize'].includes(tx.type) ? "text-light-coral" : "text-white/40"}`}>
                  ...
                  <div className="flex flex-col items-center justify-center py-32 bg-white/5 backdrop-blur-xl rounded-[3rem] border border-dashed border-white/10">
                    ...
                    <Button variant="link" onClick={() => { setSearch(""); setTypeFilter("all"); setStatusFilter("all"); }} className="text-light-coral font-bold mt-4 text-[10px] tracking-widest uppercase">
                      CLEAR ALL FILTERS
                    </Button>
                  </div>
                )}
